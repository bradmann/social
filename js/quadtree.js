$(function() {
	...
	function drawSpring(ctx, n1, n2, weight) {
	...
	function render() {
		...
		for (var i=0; i < _nodes.length; i++) {
			var node = _nodes[i];
			drawCircle(_ctx, node["x"], node["y"], node["m"] * _nodeRadius, '#000000', node["c"]);
		}
		
		for (var i=0; i < _links.length; i++) {
			var link = _links[i];
			drawSpring(_ctx, _nodes[link["a"]], _nodes[link["b"]], 1);
		}
		_ctx.restore();
	}
